...
export BARE_METAL_PROVISIONER_INTERFACE="${BARE_METAL_PROVISIONER_INTERFACE:-eth0}"
...
if [[ "${IRONIC_TLS_SETUP}" == "true" ]]; then
  ...
  if [[ ! -f "${IRONIC_CERT_FILE}" ]]; then
    openssl req -x509 -newkey rsa:4096 \
      ...
      -days 365 -nodes \
      -subj "/CN=ironic" \
      -addext "subjectAltName=IP:172.22.0.1,IP:192.168.111.1"
  fi
...
add_nft_rule() {
  ...
  if ! sudo nft list chain ip filter DOCKER-USER 2>/dev/null | grep -q "$1"; then
