...
type ScaleSpecInput struct {
	...
	// If not specified, this is a no-op.
	PostScaleClusterNamespaceCreated func(clusterProxy framework.ClusterProxy, clusterNamespace string, clusterName string, clusterClassYAML []byte, clusterTemplateYAML []byte) (clusterClassTemlate []byte, ClusterTemplate []byte)
...
func ScaleSpec(ctx context.Context, inputGetter func() ScaleSpecInput) {
	...
	BeforeEach(func() {
		...
		Expect(input.E2EConfig.Variables).To(HaveKey("KUBERNETES_VERSION"))
	...
	It("Should create and delete workload clusters", func() {
		...
		Logf("Generating YAML for base Cluster and ClusterClass")
		...
		Logf("Extract ClusterClass and Cluster from template YAML")
		...
		clusterNameDigits := 3 + int(math.Log10(float64(clusterCount)))
		...
		if input.SkipWaitForCreation {
			if !input.SkipCleanup {
				Logf("WARNING! Using SkipWaitForCreation=true while SkipCleanup=false can lead to workload clusters getting deleted before they are fully provisioned.")
			}
		...
		if err != nil {
			...
			Logf("Failed to create clusters. Error: %s", err.Error())
			Fail("")
		}
		...
		if !input.SkipUpgrade {
			...
			upgrader := getClusterUpgradeAndWaitFn(framework.UpgradeClusterTopologyAndWaitForUpgradeInput{
				ClusterProxy:                input.BootstrapClusterProxy,
				KubernetesUpgradeVersion:    input.E2EConfig.GetVariable("KUBERNETES_VERSION"),
			...
			if err != nil {
				...
				// that will contain the full stack trace of failure for each cluster to help with debugging.
				// TODO(ykakarap): Follow-up: Explore options for improved error reporting.
				Logf("Failed to upgrade clusters. Error: %s", err.Error())
				Fail("")
			}
		}
		...
		if err != nil {
			...
			// that will contain the full stack trace of failure for each cluster to help with debugging.
			// TODO(ykakarap): Follow-up: Explore options for improved error reporting.
			Logf("Failed to delete clusters. Error: %s", err.Error())
			Fail("")
		}
...
func getClusterCreateFn(clusterProxy framework.ClusterProxy) clusterCreator {
	return func(ctx context.Context, namespace, clusterName string, clusterTemplateYAML []byte) {
		Logf("Applying the cluster template yaml of cluster %s", klog.KRef(namespace, clusterName))
		Eventually(func() error {
...
type PreClusterCreateCallback func(clusterProxy framework.ClusterProxy, clusterNamespace string, clusterName string, clusterClassYAML []byte, clusterTemplateYAML []byte) (clusterClassTemlate []byte, ClusterTemplate []byte)
...
func createClusterWorker(ctx context.Context, clusterProxy framework.ClusterProxy, inputChan <-chan string, resultChan chan<- workResult, wg *sync.WaitGroup, defaultNamespace string, deployClusterInSeparateNamespaces bool, baseClusterClassYAML, baseClusterTemplateYAML []byte, create clusterCreator, PostNamespaceCreated PreClusterCreateCallback) {
	...
	for {
			...
			case clusterName, open := <-inputChan:
				...
				Logf("Creating cluster %s", clusterName)
				...
				hookClusterClassYAML := baseClusterClassYAML
				...
				if PostNamespaceCreated != nil {
					Logf("Calling PostNamespaceCreated for namespace %s", namespaceName)
					hookClusterClassYAML, hookClusterTemplateYAML = PostNamespaceCreated(clusterProxy, namespaceName, clusterName, baseClusterClassYAML, baseClusterTemplateYAML)
				}
				if deployClusterInSeparateNamespaces {
					Logf("Apply ClusterClass in namespace %", namespaceName)
					clusterClassYAML := bytes.Replace(hookClusterClassYAML, []byte(scaleClusterNamespacePlaceholder), []byte(namespaceName), -1)
					Eventually(func() error {
...
func deleteClusterAndWaitWorker(ctx context.Context, inputChan <-chan string, resultChan chan<- workResult, wg *sync.WaitGroup, c client.Client, defaultNamespace string, deployClusterInSeparateNamespaces bool) {
	...
	for {
			...
			case clusterName, open := <-inputChan:
				...
				Logf("Deleting cluster %s", clusterName)
...
func upgradeClusterAndWaitWorker(ctx context.Context, inputChan <-chan string, resultChan chan<- workResult, wg *sync.WaitGroup, defaultNamespace string, deployClusterInSeparateNamespaces bool, clusterTemplateYAML []byte, upgrade clusterUpgrader) {
	...
	for {
			...
			case clusterName, open := <-inputChan:
				...
				Logf("Upgrading cluster %s", clusterName)
