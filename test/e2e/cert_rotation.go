...
func certRotation(ctx context.Context, inputGetter func() CertRotationInput) {
	...
	clusterClient := input.TargetCluster.GetClient()
	...
	preTLSVersion, preTLSVerErr := GetDeploymentTLSSecretVersion(ctx, deployment, input.TargetCluster)
	if preTLSVerErr != nil {
		Logf("Warning: could not read tls-secret-version pre-rotation: %v", preTLSVerErr)
	}
	preRevision, preRevErr := GetDeploymentRevision(ctx, deployment, input.TargetCluster)
	if preRevErr != nil {
	...
	err := FetchClusterLogs(input.TargetCluster, filepath.Join(clusterLogCollectionBasePath, "before-cert-rotation"))
	if err != nil {
		Logf("Error: %v", err)
	}
	...
	postTLSVersion, postTLSVerErr := GetDeploymentTLSSecretVersion(ctx, deployment, input.TargetCluster)
	Expect(postTLSVerErr).ToNot(HaveOccurred())
	postRevision, postRevErr := GetDeploymentRevision(ctx, deployment, input.TargetCluster)
	...
	By("Fetch logs from target cluster after pivot")
	err = FetchClusterLogs(input.TargetCluster, filepath.Join(clusterLogCollectionBasePath, "after-cert-rotation"))
	if err != nil {
		Logf("Error: %v", err)
	}
